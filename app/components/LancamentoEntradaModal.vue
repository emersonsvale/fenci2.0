...
/**
 * LancamentoEntradaModal - Modal para criar/editar entrada (receita)
 */
...
const modalTitle = computed(() => isEditing.value ? 'Editar entrada' : 'Nova entrada')
...
// Também observar mudanças no editData quando o modal já está aberto
...
function handleSubmit(createAnother: boolean = false) {
  ...
  emit('submit', { ...form.value }, createAnother, editingId.value || undefined)
...
<template>
  <BaseModal :is-open="isOpen" :title="modalTitle" @close="emit('close')">
    <form id="lancamento-entrada-form" class="space-y-5" @submit.prevent="handleSubmit(false)">
      <!-- Conta e Categoria -->
      <div class="grid grid-cols-2 gap-4">
        <!-- Conta -->
        <div>
          <label class="label-dark">
            Conta <span class="text-primary">*</span>
          </label>
          <select
        ...
        <div>
          ...
          >
            <option value="" class="bg-surface-elevated">Selecione</option>
            <option v-for="category in incomeCategories" :key="category.id" :value="category.id" class="bg-surface-elevated">
              {{ category.name }}
            </option>
          </select>
          <button
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <div>
          <label class="label-dark">
            Sobre <span class="text-primary">*</span>
          </label>
          <input
        ...
        <div>
          <label class="label-dark">
            Valor <span class="text-primary">*</span>
          </label>
          <div class="relative">
      ...
      <div class="relative">
        <label class="label-dark flex items-center gap-1">
          Tags
          <span class="material-symbols-outlined text-xs cursor-help" title="Adicione tags para organizar (ex: freela, dividendos)">info</span>
        </label>
        ...
        <div class="flex items-end pb-2">
          <label class="flex items-center gap-3 cursor-pointer" @click="form.isRecurring = !form.isRecurring">
            <div
              ...
              :class="form.isRecurring ? 'bg-primary' : 'bg-border-dark'"
            >
              <span
            ...
            <div>
              ...
              <p v-if="form.isRecurring" class="text-xs text-content-subtle">
                Lançamento será repetido {{ form.totalInstallments }}x
              </p>
            </div>
          </label>
        </div>
      ...
      <!-- Marcar como pago -->
      <div>
        <label class="flex items-center gap-3 cursor-pointer" @click="form.isPaid = !form.isPaid">
          <div
            class="relative w-12 h-7 rounded-full transition-colors duration-200"
            :class="form.isPaid ? 'bg-primary' : 'bg-border-dark'"
          >
            <span
              class="absolute top-1 w-5 h-5 rounded-full bg-white shadow transition-transform duration-200"
              :class="form.isPaid ? 'translate-x-6' : 'translate-x-1'"
            />
          </div>
